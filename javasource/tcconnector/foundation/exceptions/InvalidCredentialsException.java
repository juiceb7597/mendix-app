...
/**
 * Exception returned from the Teamcenter server for for invalid credentials.
 */
public class InvalidCredentialsException extends AuthException {
	 ...
	 */
	public InvalidCredentialsException(JSONObject jsonObj) {
	...
	private static void validateKey(JSONObject jsonObj, String key) {
		...
		String message = "The JSON does not have the expected " + key
				+ " key, and thus cannot be parsed as an InvalidCredentialsException.";
	...
	private static String getMessage(JSONObject jsonObj) {
		...
		if (qName.equals(Constants.QNAME_CREDENTIAL_EXP))
			return !jsonObj.isNull(KEY_MESSAGE) ? jsonObj.getString(KEY_MESSAGE)
					: Integer.toString(jsonObj.getInt(KEY_CODE));

		String message = "The JSON " + Constants.KEY_QNAME + " value is incorrect for an InvalidCredentialsException ("
				+ qName + ").";
		Constants.LOGGER.error(message);
		throw new IllegalArgumentException(message);
	}
